...
* **基本用法**：通过调用 `thread::spawn` 并传递一个闭包（`|| { ... }`）来创建新线程。这个闭包包含了你希望在新线程中执行的全部代码逻辑。
* **生命周期规则**：Rust 程序中一个至关重要的规则是，当 `main` 主线程执行结束时，整个程序会立即终止，所有由它创建的子线程也会被强制关闭，无论它们是否完成了自己的任务。
* **执行顺序的不确定性**：主线程和子线程是并发执行的，它们的执行顺序由操作系统调度决定，因此是不确定的。每次运行程序，输出的交错顺序都可能不同。`thread::sleep` 函数可以用来主动让出 CPU，增加线程切换的机会，从而更清晰地观察到并发效果。
* **等待子线程完成**：为了解决主线程提前退出导致子线程被中断的问题，可以使用 `thread::spawn` 返回的 `JoinHandle`。调用其 `.join()` 方法会阻塞当前线程（通常是主线程），直到该 `JoinHandle` 对应的子线程执行完毕。这是确保子线程任务完整执行的标准方法。

**向线程传递复杂逻辑与数据**

* **封装逻辑**：当新线程需要执行的逻辑很复杂时，最佳实践是将其封装在一个独立的函数中。这样做可以使代码更加清晰、模块化、可复用且易于测试。
* **闭包的角色**：在这种模式下，传递给 `thread::spawn` 的闭包充当了一个轻量级的“胶水”或“适配器”。它的主要职责是捕获其环境中的变量，然后调用那个封装好的核心逻辑函数，并将这些变量作为参数传递过去。
* **`move` 关键字**：当子线程需要使用主线程环境中的数据时，必须在闭包前使用 `move` 关键字。`move` 会强制闭包获取其所使用的外部变量的**所有权**，将数据从主线程“移动”到子线程。这是保证线程安全的关键，因为它防止了主线程在子线程使用数据时意外地修改或销毁它。
* **返回结果**：闭包可以有返回值。主线程可以通过调用 `JoinHandle` 的 `.join().unwrap()` 来获取子线程执行完毕后返回的结果。

**多线程环境下的所有权**

* **核心原则**：Rust 的所有权模型在并发中依然有效——一个数据在同一时间点只能有一个所有者。
* **所有权转移 (`move`)**：这是最直接的方式，一个线程将数据的所有权完全转移给另一个线程。发送方在转移后将无法再访问该数据。
* **共享只读访问 (`Arc<T>`)**：当多个线程需要读取同一份数据时，使用 `Arc<T>`（原子引用计数）。`Arc` 允许多个所有者共享数据。通过克隆 `Arc` 指针（`Arc::clone()`），可以让多个线程安全地持有对同一份数据的引用。它内部的计数器是原子操作，保证了线程安全。
* **共享读写访问 (`Arc<Mutex<T>>`)**：当多个线程需要修改同一份数据时，需要结合使用 `Arc` 和 `Mutex`（互斥锁）。`Arc` 负责让多个线程安全地共享对 `Mutex` 的所有权，而 `Mutex` 则像一个保险箱，通过 `.lock()` 方法确保在同一时刻只有一个线程能访问和修改其内部的数据，从而避免数据竞争。

**Actor 模型**

* **核心思想**：Actor 模型是一种并发计算模型，其核心理念是“通过通信来共享内存，而不是通过共享内存来通信”。它从设计上避免了传统并发编程中复杂的锁和共享状态问题。
* **Actor 的定义**：一个 Actor 是一个独立的计算单元，它封装了**私有状态**和**行为**，并通过一个**邮箱 (Mailbox)** 来接收和处理来自其他 Actor 的异步消息。
* **工作方式**：Actor 的生命就是不断地从其邮箱中顺序地取出一个消息并进行处理。处理消息时，它可以改变自己的内部状态、向其他 Actor 发送新消息，或者创建新的 Actor。
* **安全性**：由于 Actor 之间的状态是完全隔离的，不能直接访问，因此从根本上杜绝了数据竞争。所有交互都必须通过消息传递完成，这提供了一种内置的、更高级别的同步机制。

**使用 `mpsc::channel` 进行消息传递**

* **创建通道**：调用 `mpsc::channel()` 会创建一个异步的、有缓冲的消息通道，并返回一个元组，包含通道的两端：发送端 (`tx`) 和接收端 (`rx`)。
* **发送端 (`tx`) 与接收端 (`rx`)**：可以将其比作一条单向流淌的河流。`tx` 是上游，负责发送消息（往河里放东西）；`rx` 是下游，负责接收消息（从河里捞东西）。这种职责分离的设计保证了类型安全和线程安全。
* **所有权与线程安全**：你可以安全地将发送端 `tx` 的所有权 `move` 到其他线程，而将接收端 `rx` 留在主线程。数据通过 `tx.send(data)` 发送时，其所有权会从发送方转移到通道中，最终由接收方通过 `rx.recv()` 取出并获得所有权。
* **`mpsc` 的含义**：`mpsc` 代表 **m**ultiple **p**roducer, **s**ingle **c**onsumer（多个生产者，单个消费者）。这意味着发送端 `tx` 可以被克隆（`tx.clone()`），从而允许多个线程向同一个通道发送消息。而接收端 `rx` 是唯一的，不能被克隆，保证了所有消息由一个消费者按顺序处理。

**`Mutex`的所有权与共享问题**

标准的 `Mutex<T>` 本身具有唯一所有权。在循环中创建多线程时，如果尝试使用 `move` 关键字将同一个 `Mutex` 移入多个线程，代码将无法编译。因为在第一次循环中，`Mutex` 的所有权就已经被**移动（move）**到第一个线程中，后续的线程无法再访问它。

**使用 `Arc<Mutex<T>>` 实现线程安全共享**

为了在多个线程间安全地共享可变数据，Rust 的标准模式是 `Arc<Mutex<T>>`。

* **`Arc<T>` (Atomically Referenced Counter)**：是一种原子引用计数智能指针。它允许一个值拥有多个所有者，并通过原子操作来管理引用数量，因此可以安全地在线程间共享。
* **工作流程**：
    1.  将 `Mutex` 包裹在 `Arc` 中：`let data = Arc::new(Mutex::new(...));`。
    2.  每当需要将访问权交给一个新线程时，克隆 `Arc`：`let data_clone = Arc::clone(&data);`。
    3.  `Arc::clone()` 的操作非常轻量，它仅仅是复制一个指向堆上相同数据的指针，并以原子方式增加引用计数，并不会克隆内部的数据（即 `Mutex`）。
    4.  将克隆出的 `data_clone` 移入新线程。

**`Arc` 与 `Mutex` 如何协同保证互斥性**

`Arc` 解决了所有权共享的问题，而 `Mutex` 负责保证访问的互斥性。即使有多个 `Arc` 的克隆体，它们全部指向**同一个、唯一的 `Mutex` 实例**。因此，无论哪个线程调用 `.lock()` 方法，它们都在尝试锁定这同一个互斥锁。`Mutex` 自身的机制确保了在任何时刻，只有一个线程能够成功获得锁并访问内部数据。

* **`Arc` 的角色**：管理共享**所有权**（Ownership）。
* **`Mutex` 的角色**：管理互斥**访问**（Access）。

**“原语”的核心概念**

“原语”（Primitive）在计算机科学中指一个系统中**基础、不可再分的最小单元或操作**。其具体含义依赖于上下文。

* **在并发和操作系统中**：原语指**原子操作（Atomic Operation）**。这是一个在执行过程中绝对不会被其他线程中断的操作，是实现所有同步机制（如锁、信号量）的基石。例如 `mutex.lock()` 就可被视为一个原语。
* **在编程语言中**：原语通常指**基本数据类型（Primitive Data Types）**，如 `i32`, `f64`, `bool`, `char` 等，它们是语言内置的，而非由其他类型组合而成。

**`i32` 与 `AtomicI32` 的对比**

`AtomicI32` 是 `i32` 的原子版本，专为多线程环境设计。

* **线程安全**：`i32` 不是线程安全的，并发读写会导致数据竞争。`AtomicI32` 是线程安全的。
* **操作方式**：`i32` 使用标准的算术运算符（`+`, `-`, `=`）。`AtomicI32` 必须使用其提供的原子方法，如 `load()`（原子读）、`store()`（原子写）、`fetch_add()`（原子加法）等。
* **使用场景**：
    * `i32`：用于单线程计算，或在多线程中被 `Mutex` 等锁保护的场景。
    * `AtomicI32`：用于实现高性能的并发计数器、状态标志等，开销通常远小于 `Mutex`。
    * `Mutex<i32>`：当需要保护的不仅仅是单次读写，而是一个**复杂的操作事务**（例如“先检查、再计算、后写入”）时使用。

**`Mutex` 的死锁风险与规避**

死锁是并发编程中的一种逻辑错误，`Arc<Mutex<T>>` 只是提供了可能导致这种错误的工具。典型的死锁发生在两个或多个线程以**不同的顺序**尝试获取两个或多个锁，导致它们陷入无限的相互等待。

* **死锁场景示例**：
    * 线程 A：锁定资源 1，然后尝试锁定资源 2。
    * 线程 B：锁定资源 2，然后尝试锁定资源 1。
* **规避策略**：规避此类死锁最有效的方法是**强制所有线程始终以相同的、全局预先规定好的顺序来获取锁**。例如，无论何时，都必须先锁定资源1，再锁定资源2。这样就不会形成循环等待，从根本上避免了死锁。程序员有责任设计并遵守这个加锁协议。
## day13

 **并行与并发**

* **核心区别**：并发 (Concurrency) 是**逻辑上同时**处理多个任务，任务之间可以交替执行，宏观上看起来像在同时发生。并行 (Parallelism) 是**物理上同时**执行多个任务，需要多核处理器支持。
* **比喻**：并发就像一个咖啡师（单核CPU）在冲咖啡、打奶泡、拿点心三件事之间快速切换，让三个任务都在推进。并行就像三个咖啡师（多核CPU）在同一时刻分别做这三件事。
* **关系**：并行一定是并发，但并发不一定并行。一个程序必须被设计成并发的，才有可能在多核硬件上并行执行。

**`async` 与 `await` 关键字**

* **`async`**：一个标记，用于声明一个函数是“异步的”。调用 `async` 函数不会立即执行并阻塞，而是会立刻返回一个“凭证”（即 `Future`）。它好比给管家一张可以“暂停和恢复”的任务清单。
* **`await`**：只能在 `async` 函数内部使用。它是一个“暂停点”。当遇到 `await` 一个尚未就绪的 `Future` 时，当前任务会暂停，并**立即交出CPU控制权**给事件循环，让其他任务得以运行。它不是阻塞式的“死等”，而是一种非阻塞的、高效的等待。
* **核心关系**：我们**不是“等待事件完成”之后才交出控制权**；恰恰相反，`await` 这个动作本身就是在发现事件尚未完成时“交出控制权”的瞬间。

**`async` 函数中的同步代码**

* **执行方式**：在一个 `async` 函数内部，位于 `await` 之间或前后的所有非 `await` 代码，都属于**同步代码块**。这些代码块在执行时，会像普通同步函数一样**阻塞当前任务**，直到该代码块执行完毕。
* **核心模型**：一个 `async` 函数，可以被看作是被 `await` 关键字**劈分**成了若干个同步代码块的集合。`async` 关键字本身并不会让里面的同步代码变得异步。
* **比喻**：管家（事件循环）在执行一张异步菜谱时，遇到“精心切水果2分钟”这个同步步骤，他必须专心致志地完成它，期间无法处理任何其他事务。

**事件循环 (Event Loop)**

* **定义**：异步编程的“心脏”和“总调度师”。它是一个在后台持续运行的循环，负责接收任务、执行任务、并在没有任务可执行时轮询外部事件（如I/O、定时器）。
* **关键队列**：
    * **任务队列 (Task Queue / Macrotask Queue)**：存放待处理的宏任务，如 `setTimeout` 回调、已完成的I/O事件等。事件循环每次循环通常只取出一个执行。
    * **微任务队列 (Microtask Queue)**：存放更高优先级的微任务，主要是 `Promise` 的回调（`await` 的后续代码）。在一个宏任务执行完后，会立即清空所有微任务。
* **比喻**：事件循环就是那位聪明的“管家”，他手上有“立即可做”和“等待中”两个记事本，不断地在执行、检查、调度之间循环，让整个系统高效运转。

**`Future` 凭证**

* **定义**：一个代表“未来某个时刻才会完成的操作的结果”的对象。调用 `async` 函数会立即返回一个 `Future`。
* **生命周期**：它有几种状态，最核心的是：
    1.  **进行中 (Pending)**：初始状态，操作尚未完成。
    2.  **已完成 (Resolved)**：操作成功，`Future` 内部持有结果值。
    3.  **已失败 (Rejected)**：操作失败，`Future` 内部持有错误信息。
* **比喻**：`Future` 就像去咖啡店点单后拿到的那个**订单小票**或**取餐器**。它本身不是咖啡，但承诺你最终能凭它换取一杯咖啡（或一个坏消息）。`await` 就是你盯着取餐器等待它震动的过程。`await` 关键字的作用，就是将一个代表未来结果的“凭证” (`Future`)，最终“兑换”成一个实际的数据。

这个“兑换”过程可以更精确地理解为一次分工合作：

1. **`Future` (凭证/容器)**：它是一个状态容器，初始状态为“进行中”，并承诺在完成后内部会包含一个实际的结果。
2. **事件循环 (后台填充者)**：它在后台监控真正的操作（如I/O、定时器）。当操作完成后，它负责将结果数据**放入** `Future` 容器中，并将其状态更新为“已完成”。
3. **`await` (拆包/兑换员)**：它是您代码中的操作。它首先暂停您的任务以等待 `Future` 被填充，然后在被事件循环唤醒后，负责从 `Future` 这个容器中**取出**最终的实际数据，并将其作为表达式的结果返回。
简单来说，`await` 委托事件循环去完成一项任务并填充凭证，然后它负责从凭证中拆包取出最终的成果。这个机制让我们可以用看似同步的简单代码，来获取异步操作的最终结果。

**并发执行多个 Future (`join`)**

* **作用**：这是一个并发工具，用于将多个独立的 `Future` “打包”成一个更高阶的 `Future`。这个新的 `Future` 会在所有内部的 `Future` 都完成后才算完成。
* **目的**：实现真正的并发执行，而不是顺序执行。使用 `join`，总耗时由耗时最长的那个任务决定，而不是所有任务耗时之和。
* **比喻**：你给管家下达一个“复合指令”，要求他同时推进烤面包和烧水两项任务，并等待这两项**都**完成后再向你汇报。

**竞速执行多个 Future (`race` 与超时)**

* **作用**：与 `join` 不同，`race` 也是将多个 `Future` 打包成一个，但它的完成条件是：**任何一个**内部的 `Future` 率先完成，整个 `race` 就立刻以这个获胜者的结果为结果而完成，其他未完成的 `Future` 会被丢弃。
* **经典应用**：实现超时控制。通过让你的主任务 `Future` 和一个 `sleep` 定时器 `Future` 进行“竞赛”，如果主任务先完成则成功，如果定时器先完成则意味着超时。

**主动让出控制权 (`yield_now`)**

* **作用**：立即暂停当前任务，并将其放入事件循环“立即可做”队列的末尾，从而让其他任务有机会运行。
* **与 `sleep` 的区别**：
    * **意图**：`yield_now` 的意图是“合作与公平”，用于打断长计算任务，防止其饿死其他任务。`sleep` 的意图是“强制等待”，任务本身需要暂停一段时间。
    * **机制**：`yield_now` 将任务直接放回就绪队列队尾。`sleep` 将任务放入一个专门的“等待定时器”列表，等时间到了才移回就绪队列。
* **比喻**：`yield_now` 是管家在做一件长工作时，主动抬头问“有急事吗？”，而 `sleep` 是管家看了一眼表，说“我必须休息5分钟，别打扰我”。

**异步与同步的交互 (`run`/`block_on`)**

* **两个世界**：程序可以被看作存在于“同步世界”（如 `main` 函数）和“异步世界”（由事件循环管理）中。
* **桥梁**：`run` 或 `block_on` 函数是从同步世界进入异步世界的“传送门”或“桥梁”。
* **阻塞的必要性**：当同步世界调用 `run` 启动异步流程时，它必须在“传送门”口**阻塞等待**，直到异步世界里的所有任务都执行完毕，`run` 函数返回一个最终结果。这个阻塞是连接两个世界的必要机制。

**`Pin` 与 `Box`**

* **背景问题**：1. 不同 `async` 块是不同类型，无法直接放入 `Vec`。2. `async` 状态机可能包含自我引用，不能在内存中随意移动。
* **`Box::pin` 方案**：
    * **`Box`**：通过在堆上分配内存，返回一个固定大小的指针，再结合 `dyn Future` Trait Object，解决了**类型对齐**问题。
    * **`Pin`**：像“图钉”一样，将内存中的数据“钉住”，承诺不会移动它，解决了**内存安全**问题。
    * **代价**：引入了堆分配的开销。
* **`pin!` 宏方案**：一种更高性能的替代方案。它可以在**栈上**直接“钉住”数据，解决了移动问题，然后再结合对引用的类型擦除（`&mut dyn Future`），解决了类型对齐问题，全程**无需堆分配**。
```mermaid
graph TD
    %% 为节点定义ID和文本
    A["<b>开始:</b><br/>调用 runtime.run(async {...})"]
    B["<b>1. 创建任务</b><br/>放入“就绪”列表"]
    C["<b>2. 从“就绪”列表取出任务</b>"]
    D["<b>3. 执行任务代码</b>"]
    E{"<b>遇到 .await</b>"}
    F["<b>4a. 任务暂停</b><br/>进入“等待I/O”状态"]
    G["<b>4b. 运行时监视I/O事件</b>"]
    H["<b>5. 任务被唤醒</b><br/>移回“就绪”列表"]
    I["<b>6. 任务完成</b>"]
